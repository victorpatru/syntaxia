...
export const Route = createFileRoute("/_authed/behavioral/session/$sessionId")({
  component: BehavioralSession,
});
...
function BehavioralSession() {
  ...
  const toggleRecording = async () => {
    ...
    setVoiceState((prev) => ({ ...prev, isRecording: newRecordingState }));
  ...
  const startVoiceConnection = useCallback(async () => {
    ...
    const dynamicVariables = {
      ...
      user_name: userProfile?.firstName || "Candidate",
      category: session.behavioralCategory || "Leadership",
      behavioral_question:
        session.behavioralQuestion ||
        (session.questions?.[0]?.text ??
          "Tell me about a time you led through ambiguity—what did you do and what was the outcome?"),
    } as const;
    try {
  ...
  useEffect(() => {
    ...
    const shouldStartVoice =
      ["setup", "active"].includes(session.status) &&
      session.questions &&
      session.questions.length > 0 &&
      !hasStartedActive;
    if (shouldStartVoice) startVoiceConnection();
  }, [
    session,
    navigate,
    isInterviewActive,
  ...
  const endInterview = useCallback(async () => {
    ...
    endMutation({ sessionId, elevenlabsConversationId: conversationId })
      .then((res) => {
        ...
        navigate({
          to: "/behavioral/analysis/$sessionId",
          params: { sessionId },
        });
      })
      .catch(() => {
